...
# В Render этот код будет выполняться,
# но мы используем os.getenv для получения реальных переменных окружения.
# В локальной разработке можно использовать заглушки
...
# Базовый URL для webhook. Render предоставляет его в переменной RENDER_EXTERNAL_HOSTNAME
WEBHOOK_URL = os.getenv("RENDER_EXTERNAL_HOSTNAME")
# Если токен не установлен, выходим
if not TELEGRAM_TOKEN or not WEBHOOK_URL:
    logging.error("TELEGRAM_TOKEN или RENDER_EXTERNAL_HOSTNAME не установлены.")
    # Присваиваем фиктивные значения для локальной работы без деплоя
...
try:
    if firebase_config_str:
        # Firebase Admin SDK требует учетные данные в виде JSON-файла или объекта.
        # Мы используем dict, если конфигурация предоставлена как JSON-строка.
        ...
        # Если конфигурация не предоставлена (например, в локальном режиме без ключа)
...
try:
    ...
    db = None # Присваиваем None, если не удалось
...
def get_patient_doc_ref(user_id: int):
    ...
    # Правило именования: /artifacts/{appId}/users/{userId}/patient_data/{docId}
    # Используем user_id в качестве docId, так как это данные, уникальные для пользователя.
    if db:
...
def get_survey_collection_ref(user_id: int):
    ...
    # Правило именования: /artifacts/{appId}/users/{userId}/survey_entries
    if db:
...
async def save_survey_data(user_id: int, survey_data: dict):
    ...
    if not db:
        logger.error("База данных Firestore недоступна.")
        return False
...
def get_main_menu_keyboard():
    ...
    # Клавиатура разделена на два ряда: "Анкета" и "Информация"
    keyboard = [
...
def main() -> None:
    ...
    # Эта часть должна быть в конце main()
    
    # Указываем, что нужно слушать Webhook на порту, который предоставит Render
    ...
    # base_url должен быть вида https://telega-bot-jtfb.onrender.com
    application.run_webhook(
        listen="0.0.0.0",
        port=port,
        url_path="", # Пустой путь, так как Webhook_url уже содержит полный путь

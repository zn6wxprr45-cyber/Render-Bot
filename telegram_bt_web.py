# -*- coding: utf-8 -*-
"""Код бота для Render Web Service

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ZeeXGAdZtIS0pLibkZ2TfCElER-BofPB
"""
...
Этот бот адаптирован для запуска на Render в режиме Web Service
(постоянно слушает HTTP-порт, но использует polling для Telegram).
"""
...
appId = os.environ.get("APP_ID", "default-app-id")
...
def initialize_firebase():
    ...
    global db, appId
    try:
        # Пытаемся получить конфиг из переменной окружения FIREBASE_CREDENTIALS
        firebase_config = json.loads(firebase_config_str)

        if not firebase_config or not firebase_config.get('project_id'):
            logger.critical(
                "Переменная FIREBASE_CREDENTIALS не содержит корректную JSON-строку."
                " Попытка использовать Application Default Credentials."
            )
            # Если не найдено, пробуем Application Default Credentials (хотя в Render это не сработает)
            cred = credentials.ApplicationDefault()
        else:
            # Используем конфигурацию из FIREBASE_CREDENTIALS
            cred = credentials.Certificate(firebase_config)

        # Назначаем appId из project_id, если он не был установлен глобально
        if appId == 'default-app-id' and firebase_config.get('project_id'):
            appId = firebase_config['project_id']
            logger.info(f"App ID установлен из project_id: {appId}")
        ...
        logger.error("Ошибка декодирования FIREBASE_CREDENTIALS. Убедитесь, что это корректная JSON-строка.")
        db = None
        return False
...
# --- Основные обработчики команд (Остаются без изменений) ---
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db and user_id:
        try:
            user_ref = db.collection(f"artifacts/{appId}/users/{user_id}/profile").document("data")
            user_data = {
                "telegram_id": user_id,
    ...
    if context.job_queue:
        ...
        if not current_jobs:
            ...
            logger.info(f"Ежедневное напоминание уже установлено для chat_id: {chat_id}")
    else:
...
# --- Обработчики статической информации (без изменений) ---
...
# --- Логика Обратной связи (ConversationHandler) (без изменений) ---
...
# --- Логика Ежедневного Опроса (ConversationHandler) (без изменений) ---
...
# --- Адаптация для Web Service: HTTP-сервер-заглушка ---

class HealthCheckHandler(BaseHTTPRequestHandler):
    """
    Простой обработчик для Health Check Render.
    """
    def do_GET(self):
        self.send_response(200)
        self.send_header('Content-type', 'text/plain')
        self.end_headers()
        self.wfile.write(b"Bot is running (polling in background).")
...
def run_http_server():
    ...
    logger.info(f"HTTP Server запущен на порту {port} для Health Check Render.")
    httpd.serve_forever()
...
def main():
    ...
    # 5. Запуск HTTP-сервера для Render Health Check в отдельном потоке
    # Render Free Web Service требует, чтобы что-то слушало порт (переменная PORT)
    http_thread = threading.Thread(target=run_http_server)
    http_thread.daemon = True # Делаем поток демоном, чтобы он не блокировал выход
    http_thread.start()

    # 6. Запуск бота в режиме polling (блокирующая операция)
    logger.info("Бот запущен в режиме Polling...")
    application.run_polling(poll_interval=1.0, timeout=10) # Добавим poll_interval и timeout

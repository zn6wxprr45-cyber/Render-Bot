...
# --- ГЛОБАЛЬНЫЕ КОНСТАНТЫ И ПЕРЕМЕННЫЕ (ИЗВЛЕКАЮТСЯ ИЗ ОКРУЖЕНИЯ PYTHON) ---

# Глобальная переменная для ID приложения (извлекается из окружения)
# Используем переменную окружения CANVAS_APP_ID или по умолчанию
...
# Используем переменную окружения FIREBASE_CONFIG или пустой JSON-объект
...
def initialize_firebase():
    ...
    try:
        ...
        # Используем конфигурацию, если она предоставлена (ожидается формат Service Account)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # В Webhook-режиме ежедневные напоминания не работают, т.к. процесс не живет постоянно.
...
async def receive_feedback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if db:
        try:
            ...
            # Сохраняем в коллекцию feedback, доступную только для этого пользователя
...
async def q5_comments_and_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Сохраняет ответ 5, весь опрос в Firestore и завершает диалог."""
    answer = update.message.text
...
def setup_application():
    ...
    if not db:
        ...
        # Вызываем исключение, чтобы Gunicorn знал об ошибке
...
# Создаем объект application, который Gunicorn будет использовать как WSGI/ASGI приложение.
# Вызов происходит при импорте модуля.
...
async def set_webhook_on_startup(app: Application):
    ...
    if not WEBHOOK_URL:
        logger.warning("WEBHOOK_URL не установлен. Webhook не будет настроен.")
        return
...
def run_once_set_webhook():
    ...
    try:
        ...
        # Если нет запущенного цикла событий, создаем новый (типично для Gunicorn)
...
# Gunicorn будет использовать глобальную переменную `application`
